...
Start by cloning the repository locally with this git command:
...
The new exe is needed because PlatinumGames executables have a limit to how many dlls can be linked to it, and if you want to link more mod dlls to the game (which is just a fancy way of saying "installing mods") you need to have some patched executable like WolfFileSizeLimitBreaker.
...
The installers for the C++ chain tools are needed because of how Automata is made, it relies on Microsoft architecture and is exclusive to Windows, it isn't native to Linux.
...
The name is a convention, dlls with that name get automatically loaded without needing to do anything extra.

`install_prerequisites.sh`:

It is easier for you to install everything this way 😊

After launching the game, SpecialK will create new folders that it needs, so don't worry if you see new folders like FAR, logs, etc...
...
The script will attempt to install the files right into the default steam
directory. If this fails you will see an error message explaining that it failed.
In this case you would need to pass the correct path to the `NieRAutomata` steam directory:

```
./install-prerequisites.sh <PATH_TO_NIER_AUTOMATA_HERE>
```

Command explanation:

- `chmod +x <file-path>` -> This makes the file executable.
...
You will need to make sure that the binaries are
executable with the following command `chmod +x <file-path>`:
